    ...
    return (
        <section className="py-24 bg-slate-50 overflow-hidden border-y border-slate-100">
            ...
            <div className="marquee-container w-full">
                <div className="marquee-content scroll-left gap-8 px-4">
                    {[...DEALS, ...DEALS].map((deal, index) => (
                        <div key={`${deal.id}-${index}`} className="w-[280px] md:w-[420px] shrink-0 bg-white rounded-[2rem] overflow-hidden shadow-lg border border-slate-100 flex flex-col group transition-all duration-500 hover:-translate-y-1">
                            ...
                            <div className="p-5 md:p-8 flex flex-col justify-between flex-1">
                                <div>
                                    <h3 className="text-md md:text-lg font-black text-navy mb-4 border-r-4 border-gold pr-3 truncate">{deal.address}</h3>
                                    <div className="grid grid-cols-2 gap-y-3 gap-x-4 mb-4">
                                        <div>
                                            <p className="text-[9px] font-black text-slate-400 uppercase">מחיר רכישה</p>
                                            <p className="text-lg font-black text-navy">{deal.price}</p>
                                        </div>
                                        <div>
                                            <p className="text-[9px] font-black text-slate-400 uppercase">תזרים נקי</p>
                                            <p className="text-lg font-black text-gold">{deal.cashFlow}</p>
                                        </div>
                                        <div>
                                            <p className="text-[9px] font-black text-slate-400 uppercase">ROI</p>
                                            <p className="text-md font-bold text-navy">{deal.roi}</p>
                                        </div>
                                        <div>
                                            <p className="text-[9px] font-black text-slate-400 uppercase">שכירות</p>
                                            <p className="text-md font-bold text-slate-600">{deal.rent}</p>
                                        </div>
                                    </div>
                                </div>
